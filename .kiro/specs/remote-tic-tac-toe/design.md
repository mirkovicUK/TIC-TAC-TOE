...
`rate_limited` and the 419 CSRF rejection come from framework middleware, and **neither is handled by the Web_Client** — an earlier draft of this paragraph said they were surfaced by Inertia's error handling, and nothing implements that. `resources/js` contains no `onError`, no `router.on`, and no status branch at all, so a 429 or a 419 leaves Inertia to its own default and the player sees no message. The correction is recorded in `docs/ai-direction.md`.

This is an accepted limitation rather than an omission to be fixed silently, and the reasoning is that no criterion asks for the client half. Requirements 10.6 and 10.7 oblige the *Game_Service* to reject with a rate-limited outcome, which it does; Requirement 10.10 obliges the service to reject a forged request and leave state unchanged, which it does; and Requirement 14.3 excludes the forgery rejection from coverage entirely. `rate_limited` correspondingly has no value in the application's vocabulary — the string appears in no enum, no route and no client module — because the 429 status *is* the outcome, produced by `ThrottleRequests` before any controller runs. `resources/js/lib/outcomes.ts` therefore omits it deliberately. What a player actually experiences is a join or move button that appears to do nothing until the window passes, and Requirement 12.13's Known Limitations section is where that belongs.
...
| `SQLITE_BUSY` | `busy_timeout` waits up to 5 s. Beyond that, the exception surfaces as a 500, and sustained contention shows up at the Health_Endpoint as the persistence layer being reported unreachable. The 5 s is a *writer's* wait: under WAL a reader takes its read mark without waiting on a writer, so the health probe's own read is off this path and stays inside Requirement 10.1's 1-second bound. |
| Persistence unreachable at `/health` | 503 with `{"status":"error","persistence":"unreachable"}` (Req 10.2). The success status is reserved for the reachable case (Req 10.1). The probe reads a named table rather than issuing `select 1`, which references no table and so reports an unreadable or schema-less database file as reachable. Two accepted limitations: the throwable is swallowed rather than reported, so an operator sees a 503 with no log line (Requirement 10 asks for records of the six Game lifecycle events, and a failed probe is not one of them); and a reader that has to recover the WAL after an unclean shutdown can take an exclusive lock and then does wait on `busy_timeout`, which is the one window where the 1-second bound is not structurally guaranteed. |
...
| Rate limit exceeded | 429 from framework middleware, before any controller runs, so no Game state changes (Req 10.6, 10.7). The Web_Client does not handle it and shows no message — an accepted limitation, since no criterion asks for the client half. See the HTTP surface section. |
...
**Decision.** Two containers. `app` runs `php:8.5-fpm`, which speaks FastCGI; `web` runs `caddy:2-alpine`, terminates TLS, serves the static assets and forwards PHP requests to `app` over port 9000.

**Alternatives.** FrankenPHP — Caddy with PHP embedded, which would be one container serving HTTPS and executing PHP in a single process, with no FastCGI hop. Also considered: nginx in place of Caddy, and running both processes in one container under a supervisor.

**Both alternatives work at this scale, and the reason is not performance.** This application is a 3x3 grid polled every two seconds by at most two browsers. Any of these arrangements would serve it with room to spare, and an argument from throughput or from static assets not occupying a PHP worker would be dressing rather than reasoning. What the choice turns on is operability.

**Reason.**

- **It is the most battle-tested PHP deployment there is.** php-fpm behind a reverse proxy is the arrangement the overwhelming majority of PHP applications run on. Every failure mode has been hit and written up by other people, which is a property of the ecosystem rather than of this code.
- **It is the safer arrangement for AI-augmented work, which is how this project was built.** A generated Dockerfile in a conventional shape can be reviewed against thousands of prior examples; a generated one in a less common shape is harder to check and harder to be sure about. The same holds for whatever a maintainer searches for when it misbehaves.
- **It is easier to debug without deep PHP experience.** The operator of this instance has none. When something fails on the box, the value of the well-trodden path is not aesthetic — it decides whether an error message has answers behind it.
- **The certificate's lifecycle stays separate from the application's.** This is the one reason specific to this project rather than general advice. ADR-009 records that Let's Encrypt rate-limits per registered domain and that `sslip.io` is one domain shared with strangers, so the certificate is the component that cannot be cheaply replaced. Attached to `web`, it is untouched by every rebuild and restart of `app`.
- **Caddy over nginx** for the reason ADR-009 already gives: automatic HTTPS in two lines of configuration. **Two containers over one supervised container** because a supervisor inside a container reintroduces process management that Compose already does, and turns two independent restart policies into one.

**What is deliberately not claimed.** That the unpublished port 9000 and the resulting "only `web` can reach php-fpm" boundary is a benefit of the split. It is not: that boundary exists because of the split and is needed because of it. A single process serving HTTPS directly has no FastCGI port to expose and no `X-Forwarded-For` to decide whether to trust. The `*` trusted-proxy range in the Rate limits section is a hazard this topology creates and then manages, not an advantage it confers.

**This was inherited rather than decided, and the record says so.** `php:8.5-fpm` came from the technology table, and the second container followed mechanically from php-fpm not speaking HTTP. The reasoning above was assembled afterwards, when the question was put directly, and it holds — but it is a justification found to be sound rather than the reasoning that produced the choice. The distinction is the difference between a decision record and a rationalisation, and `docs/ai-direction.md` carries other entries where a correct decision rested on a false stated reason.

**What would change it.** A real domain in place of `sslip.io` removes the shared rate limit and with it most of the certificate argument. On a greenfield start, or with PHP experience on the operating side, FrankenPHP would be the obvious choice and this record would read the other way. What rules it out here is timing rather than merit: adopting it now would mean validating a deployment shape nobody on this project has run, against an already-issued certificate whose storage layout would be assumed rather than known.
