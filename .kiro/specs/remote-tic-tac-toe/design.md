...
Being honest about the value: the distinction closed here carried no practical benefit to an attacker either way. A Game_Id is a UUIDv7 with roughly 74 random bits, so there is nothing to enumerate and no id to test that was not already known. The change is made so that the table and this paragraph agree, not because it closes a meaningful hole.
