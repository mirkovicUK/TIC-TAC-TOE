...
- [ ] 15. Documentation and records
  - [ ] 15.1 Write the README
    ...
    - **State that an unfiltered test run needs a browser.** `composer test` runs `php artisan test` with no group filter, so it starts Playwright and chromium; `./vendor/bin/pest --exclude-group=browser` is the run that does not. CI's quality job already excludes the group. Give both commands rather than only one, since a reader who has not run `npm ci` will otherwise see a failure that looks like a broken suite
