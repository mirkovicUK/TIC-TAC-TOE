...
- [x] 6. Play a Game: moves, and end-of-game signalling
...
- [x] 13. Containerisation and deployment
  ...
  - [x] 13.3 Deploy and schedule the sweep
    - **The command-by-command runbook was `docs/deploy-schedule-swap.md`**, written against the instance's actual state rather than from the repository: the clone at `/srv/tic-tac-toe` and the `caddy-data` certificate were already in place, and 911 MiB of RAM with no swap is why swap was step 2 rather than a troubleshooting note
    - **That file was deleted by task 9.4 of the `continuous-deployment` spec**, because roughly 70% of it described building the image on the instance, which the pipeline replaced. It is in git history if the sequence used here matters; `docs/deployment.md` is the current document, and the parts of the old one still worth having — never change `APP_KEY`, what survives a deployment, and verifying the lifecycle records against the `moves` table — were folded into it
...
- [x] 15. Documentation and records
  ...
  - [x] 15.2 Write the decision records under `docs/decisions/`
  ...
  - [x] 15.3 Complete the AI-direction record
...
- [x] 16. Final checkpoint - Ensure all tests pass, ask the user if questions arise.
  ...
  - **All five observed against commit `a22db7d`, not asserted.** `pest --exclude-group=browser`: 298 passed, 8,394 assertions. `pest --group=browser`: 1 passed, 19 assertions. `composer lint`: 105 files pass. `composer analyse`: no errors at level 8 over `app`, `database` and `tests`, and none at level `max` over the domain. `npx tsc --noEmit` clean and `npm run build` succeeds, which the feature tests depend on for the Vite manifest
  - **Both CI jobs green on that commit**, checked per job rather than by the run's summary conclusion: `quality` success, `browser` success (run 31278879550)
  - **The hosted instance answers over the hostname from task 2.1**: `https://18-175-88-107.sslip.io/health` returns `{"status":"ok","persistence":"reachable"}` with HTTP/2 200, and `/` returns 200
  - **`failOnRisky="true"` was added to `phpunit.xml` at this checkpoint** rather than left for later. PHPUnit already reported a test performing no assertions as risky, but with the flag unset the run still exited 0, so CI would have shown green over it — measured both ways against a probe test that asserts nothing. It closes only the zero-assertion class, and deliberately not the class recorded in `docs/ai-direction.md`, where seventeen assertions did execute and could not fail
  - **Mutation testing was attempted here and abandoned, which is worth recording rather than omitting.** Infection 0.34.2 does not interoperate with this stack: it generates a PHPUnit configuration containing a legacy `<filter>` element that PHPUnit 12 rejects, so its initial test run exits 1 and it refuses to start even though all 298 tests pass inside it; and Pest registers its test classes under a `P\` namespace in the coverage XML while `junit.xml` records them unprefixed, so mutant-to-test mapping fails. Forced past both with pre-generated coverage, a `--noop` run — which mutates nothing, so every mutant must escape — reported **45 of 45 killed and 100% MSI**. A perfect score from a harness doing nothing, which is the same shape as the vacuous-assertion defect and the reason the result was checked instead of quoted. Infection was removed; PCOV stays available for `pest --coverage`
  - **A coverage run was kept as a diagnostic, not a gate.** 96.7% over `app/`, and the gaps are the useful part: the session-keyed branch of the Rate_Limit_Subject (`AppServiceProvider:137`) has never been exercised, because the suite runs with `SESSION_DRIVER=array` and every test keys on IP; `CreateGame`'s join-code collision retry and its attempt ceiling are unexercised; so are `CreateRematch`'s lost-race branch, `GameEventLogger`'s silent log-failure catch, and `Models/User`, which is Laravel scaffold for a feature ADR-005 rejected. None is a criterion violation and none was fixed here — they are recorded so the next change to those files knows what is and is not verified
