...
*Assumption to verify in the design phase, rather than a criterion:* that SQLite applies the DDL of one migration transactionally, so that a migration failing part way leaves the schema as it was rather than half-changed. Criterion 7 bounds the damage either way — nothing serves requests against that schema — but how much the Fallback_Deployment is exposed depends on it.
