...
- **Deployment_Environment**: The GitHub Actions environment the deploying job targets, named `production`, whose deployment-branch protection rule restricts it to the Deployment_Branch. It is what places the repository's identity in the subject claim of the presented token, and what enforces the branch restriction before a token is issued.
...
3. THE Trust_Policy SHALL permit assumption of the Deployment_Role only by the GitHub OpenID Connect provider registered in account `811362454196`, SHALL condition that assumption on an exact string match of the subject claim of the presented token against the single value `repo:mirkovicUK/TIC-TAC-TOE:environment:production`, and SHALL contain that subject condition with no wildcard character in its value, so that a run in any other repository and a run that does not target the Deployment_Environment are refused. The subject is scoped to the Deployment_Environment rather than to a branch reference because the two forms are mutually exclusive — a job targeting an environment presents `environment:` in its subject claim and not `ref:` — and because AWS's own guidance for a shared OIDC provider recommends environment scoping with protection rules over a bare branch condition.
3a. THE Deployment_Environment SHALL restrict its deployment branches to the Deployment_Branch alone, so that the branch restriction lost from the subject claim by criterion 3 of this requirement is enforced by GitHub before a token is issued rather than by AWS after one is presented. WHERE that restriction is absent, a run on any branch targeting the Deployment_Environment would be able to assume the Deployment_Role, so this criterion is the whole of the branch boundary and not a convenience.
3b. THE Deployment_Workflow's deploying job SHALL declare the Deployment_Environment, so that the subject claim required by criterion 3 of this requirement is the one GitHub issues.
